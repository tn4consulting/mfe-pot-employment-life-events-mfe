...
/**
 * The original life event this whole family proves the pattern on --
 * migrated from the pre-kit checklist-data.ts/GuidedJourney.tsx almost
 * verbatim. Two sections deliberately use the escape hatch rather than the
 * standard shapes: "employability" injects JobSearchChecklistItem as a
 * `leadingItem` (a real completion signal from job-bank, sitting inline
 * with otherwise self-reported items), and "ei" is `kind: 'custom'`
 * entirely (two checklist items sharing one widget instance doesn't fit
 * the single-widget-per-section shape cleanly).
 */
export const JOB_LOSS: LifeEventDefinition = {
  id: 'job-loss',
  introContentKey: 'life-events.job-loss.intro',
  sections: [
    {
      kind: 'checklist',
      id: 'departure',
    ...
    },
    {
      ...
      leadingItem: () => React.createElement(JobSearchChecklistItem),
    },
    {
      kind: 'checklist',
      id: 'benefits',
    ...
    },
    {
      kind: 'custom',
      id: 'ei',
      render: () => React.createElement(EiChecklistItems),
    },
    {
      kind: 'widget',
      id: 'payment-history',
